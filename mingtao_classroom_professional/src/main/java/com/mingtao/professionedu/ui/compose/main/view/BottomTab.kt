...
fun BottomTab(selected: Int, onSelectedChanged: (Int) -> Unit) {
    ...
    Row(Modifier.background(color_f)) {
        TabItem(if (selected == 0) R.mipmap.mtp_home_select else R.mipmap.mtp_home_normal, if (selected == 0) color_3081e4 else color_696969, "首页",
            Modifier
                .weight(1F)
                .noClickable {
                    onSelectedChanged(0)
                })
        TabItem(if (selected == 1) R.mipmap.mtp_study_select else R.mipmap.mtp_study_normal, if (selected == 1) color_3081e4 else color_696969, "学习",
            Modifier
                .weight(1F)
                .noClickable {
                    onSelectedChanged(1)
                })
        TabItem(if (selected == 2) R.mipmap.mtp_news_select else R.mipmap.mtp_news_normal, if (selected == 2) color_3081e4 else color_696969, "发现",
            Modifier
                .weight(1F)
                .noClickable {
                    onSelectedChanged(2)
                })
        TabItem(if (selected == 3) R.mipmap.mtp_user_select else R.mipmap.mtp_user_normal, if (selected == 3) color_3081e4 else color_696969, "我的",
            Modifier
                .weight(1F)
                .noClickable {
                    onSelectedChanged(3)
