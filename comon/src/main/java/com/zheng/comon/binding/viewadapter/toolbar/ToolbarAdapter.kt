package com.zheng.comon.binding.viewadapter.toolbar
...
object ToolbarAdapter {
    ...
    fun onNavigationClick(toolbar: Toolbar, listener: View.OnClickListener?) {
    ...
    fun setStatusBarDarkFont(view: View, isDark: Boolean) {
        val activity = CommonUtils.getActivityFromView(view)
        activity?.let {
            ImmersionBar.with(activity).statusBarDarkFont(isDark).init()
        }
    }
